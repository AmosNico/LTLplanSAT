...
showAction pt (Action name pre add del cost) =
  C8.concat
    [ name,
      C8.pack " (cost ",
      C8.pack (show cost),
...
perfectHash fdr = Vec.unfoldrExactN (FDR.nVars fdr + 1) f (0, 0)
  where
...
inverseHash fdr nFacts = Vec.unfoldrExactN nFacts f (0, 0)
  where
    f (var, val) =
...
deletingEffects pt a = concatMap f (FDR.actionPost a)
  where
...
convertAction pt a@(FDR.Action name pre post c) = Action name pre' add del c
  where
...
fromFDR fdr@(FDR.FDR _ mgs s g as) = STRIPS nfacts showfact as' s' g' (oldMGs ++ newMGs)
  where
    ...
    newMGs =
      [ [convertFact fdr (var, val) | val <- [0 .. FDR.domainSize fdr var - 1]]
